# Calgary MeshCore

Calgary is the initial launch region for Alberta MeshCore.

This section documents the local companion network, repeaters, observer
stations, MQTT infrastructure, dashboards and community projects serving
Calgary and surrounding areas.

## Calgary regional identifier

The identifier used for Calgary documentation and monitoring is: YYC

## Calgary MeshCore Links

- [Calgary YYC MeshMapper](https://yyc.meshmapper.net/?lat=51.01674&lon=-114.00149&zoom=11.00)
- [Calgary MeshCore Discord](https://discord.com/invite/CznDhsRWnJ)
- [Calgary MeshCore Corescope LIVE Activity](https://live.meshcore.ca/#/live)
- [Calgary MeshCore Observers](https://live.meshcore.ca/#/observers)
- [Calgary MeshCore Channels](https://live.meshcore.ca/#/channels)
- [Calgary MeshCore CartoLite](https://carto.canadaverse.org/)
- [Calgary MeshCore Beacon](https://beacon.canadaverse.org/?tab=Live&iata=YYC)
- [YYC MeshCore Beacon](https://dev.meshcore.ca/?tab=Map&iata=YYC)
- [Calgary MeshCore Krabs Lagoon](https://canadaverse.org/krab/lagoon/)
- [MeshMonitoring.com Crescope Calgary YYC](https://corescopeyyc.meshmonitoring.com/#/live)
- [Calgary MeshCore CoreComms.net](https://corecomms.net/?region=na)
- [Calgary MeshCore WAEV.app](https://waev.app/#/live-map/@51.08251,-114.08311,9.27z)
- [Calgary MeshCore MeshRank.net Chat](https://www.meshrank.net/messages?channel=chat&region=ca)
- [MeshMonitoring.com - Monitoring the Canadian & USA Mesh Networks](https://meshmonitoring.com/)
- [Interlink Radio Canada - Canadian Meshtastic & MeshCore Support Channels](https://interlinkradio.weebly.com/)
- [Canadaverse MeshCore Launch Directory](https://canadaverse.org/)
- [MeshCore Canada](https://meshcore.ca/)

## RX Channels

These are the recommended public receive (RX) channels commonly used throughout the Calgary MeshCore community. You can subscribe to any or all of them depending on your interests.

### Primary Public Channel

- `Public`

### Community

- `#yyc`
- `#calgary`
- `#airdrie`
- `#albertamesh`
- `#meshcorealberta`

### Mesh Networks

- `#meshcore`
- `#meshmonitoring`
- `#monitoring`
- `#mesh`
- `#meshtastic`
- `#corecomms`

### Weather

- `#weather`
- `#yyc-weather`
- `#wx`

### Emergency

- `#emergency`
- `#sos`

### Radio

- `#hamradio`

### Information

- `#news`
- `#sports`
- `#joke`
- `#jokes`

### Development & Testing

- `#chat`
- `#bot`
- `#wardriving`
- `#test`
- `#testing`